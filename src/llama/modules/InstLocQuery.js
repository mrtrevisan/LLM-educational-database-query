	...
	// function
	{
		InstitutionLocationQuery : async ( { sql_select, sql_where } ) => {
			...
			try {
				...
				return 'The sql query returned the following JSON: ' + JSON.stringify({sqlResult : res});
			} catch (e) {
				return 'The sql query returned an ERROR: ' + JSON.stringify({error : e.message, arguments : { sql_select, sql_where }});
			}
		}
	},
		...
		function : {
			...
			parameters: {
				...
				properties: {
					...
					sql_where : {
						type: "STRING",
						description: "WHERE clause to form the sql. Varchar columns must be wrapped with LCASE() function. Use LowerCase string literals and single quotes. For example, WHERE LCASE(NOME_IES) LIKE 'universidade%'",
					}
				},
				required : ["sql_select"]
			},
